...
class MS5837:
    ...
    async def init(self):
        try:
            self._bus.write_byte(self._addr, self._MS5837_RESET)
        except:
            print(f"Bus {self._bus} is not available.")
        ...
        for i in range(7):
            c = self._bus.read_word_data(self._addr, self._MS5837_PROM_READ + 2 * i)
    ...
    async def read(self, oversampling=OSR_8192):
        ...
        self._bus.write_byte(self._addr, self._MS5837_CONVERT_D1_256 + 2 * oversampling)
        ...
        await asyncio.sleep(2.5e-6 * 2 ** (8 + oversampling))

        d = self._bus.read_i2c_block_data(self._addr, self._MS5837_ADC_READ, 3)
        self._D1 = d[0] << 16 | d[1] << 8 | d[2]

        # Request D2 conversion (pressure)
        self._bus.write_byte(self._addr, self._MS5837_CONVERT_D2_256 + 2 * oversampling)
